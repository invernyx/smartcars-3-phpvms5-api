...
if($_GET['flightID'] === null)
{
    error(400, 'flightID is a required field (type `string`)');
    exit;
}
assertData($_GET, array('flightID' => 'int'));
...
$schedule = $database->fetch('SELECT id FROM ' . dbPrefix . 'schedules WHERE id=?', array($_GET['flightID']));
if($schedule === array())
{
...
$database->execute('INSERT INTO ' . dbPrefix . 'bids (pilotid, routeid, dateadded) VALUES (?, ?, NOW())', array($pilotID, $_GET['flightID']));
