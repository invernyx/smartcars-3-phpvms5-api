...
if($_GET['bidID'] === null)
{
    error(400, 'Bid ID is a required parameter (type `int`)');
    exit;
}
assertData($_GET, array('bidID' => 'int'));
$routes = $database->fetch('SELECT routeid FROM ' . dbPrefix . 'bids WHERE pilotid=? AND bidid=?', array($pilotID, $_GET['bidID']));
if($routes === array())
{
...
if($schedule === array())
{
    error(404, 'No schedule could be found with this bid');
    exit;
}
